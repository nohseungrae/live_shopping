...
export const GoodsBody: React.FC = () => {
  ...
  const noneGoodsStyles = useMemo(
    () => "flex h-full justify-center items-center font-medium",
    []
  );
      ...
      <div className={"box-border overflow-y-auto"} style={{ height: "400px" }}>
        <ul className={"relative goods_list_ul h-full"}>
          {goodsData?.getGoodsList?.goodsList?.length < 1 ? (
            <div
              className={noneGoodsStyles}
              style={{ paddingBottom: "20px", color: "#929294" }}
            >
              등록된 상품이 없습니다.
            </div>
          ) : (
            goodsData?.getGoodsList?.goodsList?.map(
              (item: any, index: number) => {
                const marketPrice =
                  item.goodsId.template[0].prices[0].market_price;
                  ...
                  <li key={index} className={"goods_list_li"}>
                    ...
                    >
                      ...
                      <div className={"goods_info"}>
                        <span className={"goods_info_name"}>
                          {item.goodsTitle}
                        </span>
                        <span className={"goods_info_price"}>
                          <span className={"price_percent"}>
                            {discount(marketPrice, salePrice)}
                          </span>
                          <span className={"goods_market_price"}>
                            {comma(marketPrice)}
                          </span>
                        </span>
                        <span className={"goods_info_sale_price"}>
                          {comma(salePrice)}
                        </span>
                        <span className={"goods_info_brandname"}>
                          {item.goodsId.template[0].brand.name}
                        </span>
                      </div>
                    </a>
